...
def _compute_llama3_yarn_parameters(
    config: PretrainedConfig, device: "torch.device", seq_len: Optional[int] = None, **rope_kwargs
) -> Tuple["torch.Tensor", float]:
    """
    ...
    inv_freq, attention_factor = _compute_yarn_parameters(config, device, seq_len, **rope_kwargs)
...
def forward(
    ...
    if not has_kv_cache: # prefill
        query_states, _ = apply_rotary_pos_emb_map(query_states, _ , cos, sin, has_kv_cache) # Q
        attn_output, _, _ = flash_attn_func(query_states.transpose(1, 2), 
...
def replace_with_flashattn(max_test_length):
