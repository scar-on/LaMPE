...
def forward(
    ...
    if not has_kv_cache:
        ...
        map_cos_Q, map_sin_Q = map_cos_Q*attention_factor,map_sin_Q*attention_factor
        map_cos_K, map_sin_K = map_cos_K*attention_factor,map_sin_K*attention_factor

        self.register_buffer("prefill_len", torch.tensor(position_ids.shape[1]), persistent=False)
        self.register_buffer("map_len", torch.tensor(kv_seq_len-head), persistent=False)
    ...
    if not has_kv_cache:
        ...
        # map prefill
            ...
            attn_output = flash_prefill_merge(head_out,head_lse,map_out,map_lse,head) 
        else: 
        ...
        # map decoding
        if kv_seq_len >head:
            _, mapping_ids = map_ids(torch.arange(kv_seq_len).unsqueeze(0).to(value_states.device), 0, use_length)
            ...
            map_key_states = map_cache[:,:kv_seq_len-head , :, :]  #self.re_size_2+decode_len:
            map_value_states = value_cache[:,:kv_seq_len-head , :, :] 

            _, map_key_states = apply_rotary_pos_emb_map(None, map_key_states.transpose(1,2), map_cos_K[:, :kv_seq_len-head, :]
                                                        , map_sin_K[:, :kv_seq_len-head, :], has_kv_cache)
            ...
            attn_output = flash_decode_merge(head_out,head_lse,map_out,map_lse)
        else: 
            attn_output= head_out
...
def replace_with_hmt(max_test_length,scale_factor):
