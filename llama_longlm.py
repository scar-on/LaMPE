...
class YarnRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        freqs = torch.outer(position_ids.to(dtype=torch.float32).squeeze(0), self.inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1)
        cos = (emb.cos()).unsqueeze(0).to(x.dtype)
...
def forward(
    ...
    if not has_kv_cache:
        ...
        map_cache[:, kv_seq_len - key_states.shape[-2]:kv_seq_len, :, :] = key_states.transpose(1, 2) # add map k cache
    ...
    if not has_kv_cache:
        ...
        head_out, head_lse, _ = flash_attn_func(head_query_states.transpose(1, 2), 
                                                ...
                                                causal=True, 
                                                window_size=[group_size_2, 0], #位置编码需要bias+1才能匹配
                                                return_attn_probs=True)    
        if kv_seq_len > group_size_2:
        # map prefill
            front_pos =  kv_seq_len - group_size_2
            map_query_states,_ = apply_rotary_pos_emb_map(query_states[:,:,-front_pos:,:], None, group_q_cos[:,-front_pos:,:], 
                                                        group_q_sin[:,-front_pos:,:],has_kv_cache) # map Q   
            _, map_key_states = apply_rotary_pos_emb_map(None, map_cache[:,:front_pos,:,:].transpose(1,2), group_k_cos[:,:front_pos,:], 
                                                        group_k_sin[:,:front_pos,:],has_kv_cache)  # map k bug
            ...
            attn_output = flash_prefill_merge(head_out,head_lse,map_out,map_lse,group_size_2) 
        else: 
        ...
        # head decoding
        k_cache_head = key_cache[:, kv_seq_len-group_size_2:kv_seq_len, :, :]
        v_cache_head = value_cache[:, kv_seq_len-group_size_2:kv_seq_len, :, :]     
        head_out, head_lse, _  = flash_attn_func(head_query_states.transpose(1, 2), 
                                    k_cache_head,
                                    v_cache_head, 
                                    causal=True, 
        ...
        if kv_seq_len > group_size_2:
            ...
            map_query_states, _ = apply_rotary_pos_emb_map(query_states, None, group_q_cos, group_q_sin, has_kv_cache)
            map_key_states = map_cache[:,:kv_seq_len-group_size_2 , :, :]  #self.re_size_2+decode_len:
            map_value_states = value_cache[:,:kv_seq_len-group_size_2 , :, :] 

            _, map_key_states = apply_rotary_pos_emb_map(None, map_key_states.transpose(1,2), group_k_cos[:, :kv_seq_len-group_size_2, :]
                                                        , group_k_sin[:, :kv_seq_len-group_size_2, :], has_kv_cache)
...
def replace_with_longlm(max_test_length):
