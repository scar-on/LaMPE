...
extra_args=(
    ...
    ["map"]="--head 2048 --use_length 3072 --tail 200"
)

data_dir="data-jsonl"
script="test_ruler_mistral.py"
    ...
    for method in "${methods[@]}"; do
        ...
        for task in "${tasks[@]}"; do
            ...
            CUDA_VISIBLE_DEVICES=6 python "$script" \
                --task "$task" \
                               --data_dir "$data_dir/$task/Mistral-7B-Instruct-v0.3-${MAX_len}.jsonl" \
                --method "$method" \
                --model_path "${MISTRAL_MODEL_PATH:-mistralai/Mistral-7B-Instruct-v0.3}" \
                --max_length "$MAX_len" \
                ${extra_args[$method]}
        done
    done
