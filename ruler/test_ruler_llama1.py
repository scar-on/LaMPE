...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default=os.environ.get("LLAMA_MODEL_PATH", "meta-llama/Meta-Llama-3-8B-Instruct"), type=str)
    ...
    if args.method == "map":
        print("map")
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
        from llama_sw_map import replace_with_hmt, forward # map
        from modifed_forward import modify_method_of_instance

        replace_with_hmt(max_test_length=args.max_length+max_new_tokens, scale_factor=0.1*math.log(2) + 1)
